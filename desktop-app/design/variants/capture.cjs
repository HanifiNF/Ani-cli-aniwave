...
const variants = process.argv.slice(2).length ? process.argv.slice(2) : ["a-guide", "b-palette", "b2-palette-art", "c-native"];
...
app.whenReady().then(async () => {
  ...
  for (const variant of variants) {
    ...
    if (variant === "b2-palette-art") {
      ...
      await win.webContents.executeJavaScript(`delete document.documentElement.dataset.theme; true`);
    }

  }
  nativeTheme.themeSource = "light";
  await win.loadFile(join(__dirname, "c-native.html"));
  await wait(500);
  for (const screen of ["home", "series"]) {
    await win.webContents.executeJavaScript(`location.hash = ${JSON.stringify(screen)}; true`);
    await wait(400);
    await writeFile(join(__dirname, "shots", `c-native-${screen}-light.png`), (await win.webContents.capturePage()).toPNG());
    console.log(`captured c-native-${screen}-light`);
  }
  win.destroy();
  app.quit();
