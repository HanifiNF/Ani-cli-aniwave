...
import type { AniPlayerApi } from "../shared/contracts";

const bridge = vi.hoisted(() => ({ api: undefined as unknown as AniPlayerApi,
  handlers: new Map<string, Set<(...args: any[]) => void>>(), send: vi.fn(), invoke: vi.fn() }));
vi.mock("electron", () => ({
  contextBridge: { exposeInMainWorld: (_name: string, api: AniPlayerApi) => { bridge.api = api; } },
...
const toggle = (enabled: boolean) => { for (const handler of bridge.handlers.get("player-window:diagnostics-change") ?? []) handler({}, enabled); };
beforeAll(async () => { await import("../electron/player-preload"); });
beforeEach(async () => {
...
describe("player keyboard diagnostics", () => {
  it("observes consumed and repeated keys without changing their behavior", async () => {
    ...
    try {
      ...
      expect(bridge.send).toHaveBeenCalledExactlyOnceWith("player-window:diagnostic", "session-1",
    ...
    expect(bridge.send).toHaveBeenCalledWith("player-window:diagnostic", "session-1", expect.objectContaining({ phase: "keyup", prevented: false }));
  });
